...
public final class CompressingFilter implements Filter {
	...
	private boolean isCompressablePath(final String path) {
		if (path != null) {
			for (final Pattern pattern : context.getPathPatterns()) {
	...
	private boolean isCompressableUserAgent(final String userAgent) {
		if (userAgent != null) {
			for (final Pattern pattern : context.getUserAgentPatterns()) {
